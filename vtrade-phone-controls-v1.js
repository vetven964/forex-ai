/* V TRADE AI — PHONE CONTROLS V11
 * PHONE ONLY: one timeframe selector + Analyze AI.
 * Desktop/trading logic untouched.
 * V11: deterministic removal of the floating VET VEN / Administrator identity card.
 */
(()=>{
'use strict';
if(!window.matchMedia||!matchMedia('(max-width:900px)').matches||window.__VTRADE_PHONE_CONTROLS_V11__)return;
window.__VTRADE_PHONE_CONTROLS_V11__=true;
const style=document.createElement('style');style.id='vtrade-phone-controls-v11-style';style.textContent=`
...
function hideAdminCard(){
 ...
 // 1) Known profile/account hooks: hide the actual card container, not the text only.
 for(const el of document.querySelectorAll('[id*="profile" i],[class*="profile" i],[id*="account" i],[class*="account" i]')){
  const t=textOf(el);if(!isIdentity(t))continue;
  const r=el.getBoundingClientRect();
  if(r.width>=180&&r.height>=45&&r.left>vw*0.20){hide(el);return}
 }
 // 2) Hit-test the visible lower-right card from the screenshot geometry.
 const points=[[vw-18,vh-110],[vw-25,vh-150],[vw-80,vh-80],[vw-120,vh-120]];
 for(const [x,y] of points){
  for(const el of document.elementsFromPoint?.(x,y)||[]){
   ...
   for(let i=0;i<10&&p&&p!==document.body;i++,p=p.parentElement){
    ...
    if(r.width>=180&&r.height>=45&&r.width<=vw*0.99&&r.height<=vh*0.45&&r.left>vw*0.20){hide(p);return}
   }
  }
 }
 // 3) Exact text/card scan, including fixed/absolute containers.
 const candidates=[];
 for(const el of document.querySelectorAll('body *')){
  ...
  const t=textOf(el);if(!isIdentity(t))continue;
  const r=el.getBoundingClientRect();
  if(r.width<180||r.width>vw*0.99||r.height<45||r.height>vh*0.45)continue;
  const cs=getComputedStyle(el);
  const floating=['fixed','absolute','sticky'].includes(cs.position);
  const lowerRight=r.left>vw*0.20&&r.top>vh*0.30;
  const score=(floating?100:0)+(lowerRight?50:0)+(r.width>=260?20:0)+(r.height<=160?15:0);
  candidates.push({el,r,score});
 }
 if(candidates.length){candidates.sort((a,b)=>b.score-a.score);hide(candidates[0].el);return}
 // 4) Climb from individual identity text fragments to the nearest card-sized ancestor.
 for(const start of document.querySelectorAll('body *')){
  const st=textOf(start);if(!/VET\\s+VEN|Administrator/i.test(st))continue;
  let p=start;
...
let tries=0;const run=()=>{if(!isPhone())return;hideAdminCard();wireTimeframe();if(tries++<240)setTimeout(run,250)};
...
window.addEventListener('resize',()=>setTimeout(run,50),{passive:true});
})();