/* V TRADE AI — PHONE TERMINAL SHELL V4.1
   Phone only. Desktop/PC untouched. */
(() => {
  'use strict';
  if (!matchMedia('(max-width:900px)').matches || window.__VTRADE_PHONE_V41__) return;
  window.__VTRADE_PHONE_V41__ = true;
  const style=document.createElement('style');
  style.id='vtrade-phone-v41-style';
  style.textContent=`
...
/* Absolutely remove the generated floating account/profile card from phone header. */
#vtradeAccountMenu,#vtradeProfileLink,#vtradeAdminLink,#vtradeUserLink,.vtrade-menu-head,[id*="AccountMenu"],[id*="accountMenu"],[class*="account-menu"],[class*="accountMenu"],[class*="profile-menu"],[class*="profileMenu"]{display:none!important;visibility:hidden!important;opacity:0!important;pointer-events:none!important;position:absolute!important;left:-99999px!important;top:-99999px!important;width:1px!important;height:1px!important}
...
body.vtrade-phone-menu-open #vtradeMobileBar{display:none!important}body.vtrade-phone-menu-open{overflow:hidden!important}
}
...
`;
  ...
  const isAccount=(el)=>{if(!el||el.id==='vtradeMobileBar')return false;const s=((el.id||'')+' '+(el.className||'')).toLowerCase();return s.includes('accountmenu')||s.includes('account-menu')||s.includes('profilemenu')||s.includes('profile-menu')};
  const hideFloating=()=>{
    document.querySelectorAll('body *').forEach(el=>{if(isAccount(el))el.style.setProperty('display','none','important')});
    const top=document.querySelector('.top');
    if(top) top.querySelectorAll('a,button').forEach(el=>{const t=(el.textContent||'').trim();if(/^(VET VEN|Administrator)$/i.test(t)||/VET VEN/i.test(t)&&/Administrator/i.test(t))el.style.setProperty('display','none','important')});
  };
  hideFloating();
  ...
  const syncMenu=()=>{
    ...
    const bar=document.getElementById('vtradeMobileBar');if(bar)bar.style.setProperty('display',open?'none':'grid','important');
  };
  const mo=new MutationObserver(syncMenu);if(side)mo.observe(side,{attributes:true,attributeFilter:['class']});
  syncMenu();
})();
