...
if (fs.existsSync(SERVER)) {
  ...
  if (!s.includes(MARK)) {
    ...
    // Use Structured Outputs for GPT-5.6 instead of legacy JSON mode.
    const oldFormat = "text:{format:{type:'json_object'}}";
    const newFormat = "text:{format:{type:'json_schema',name:'xau_ai_confirmation',strict:true,schema:{type:'object',additionalProperties:false,properties:{decision:{type:'string',enum:['BUY','SELL','WAIT']},confidence:{type:'number'},agreement:{type:'string',enum:['AGREE','DISAGREE','NEUTRAL']},reasons:{type:'array',items:{type:'string'}},missingConfirmations:{type:'array',items:{type:'string'}},riskFlags:{type:'array',items:{type:'string'}},summary:{type:'string'}},required:['decision','confidence','agreement','reasons','missingConfirmations','riskFlags','summary']}}}";
    ...
    console.log('[V-TRADE AI] AI Confirmation Runtime V3 installed');
  }
}
