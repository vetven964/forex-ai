...
//+------------------------------------------------------------------+
#property strict
#property version   "1.802"
#property description "VT Markets MT5 -> V-TRADE AI bridge with D1 candle data"
...
void SendSnapshot(){
   ...
   for(int attempt=1;attempt<=max_attempts;attempt++){
      ...
      if(code>=200&&code<300){g_fail_count=0;g_last_ok_ms=(uint)GetTickCount64();SaveSequence();Print("V-TRADE Bridge v1.802 OK | seq=",g_sequence," | MTF=M5,M15,H1,H4,D1 READY | ms=",elapsed);return;}
...
int OnInit(){LoadSequence();int timer_seconds=MathMax(2,InpTimerSeconds);if(!EventSetTimer(timer_seconds)){Print("V-TRADE Bridge EventSetTimer FAILED | error=",GetLastError());return INIT_FAILED;}Print("V-TRADE Bridge v1.802 STARTED | MTF=M5,M15,H1,H4,D1 | Timer=",timer_seconds,"s | Symbol=",BridgeSymbol());return INIT_SUCCEEDED;}
void OnDeinit(const int reason){SaveSequence();EventKillTimer();Print("V-TRADE Bridge v1.802 STOPPED | reason=",reason," | lastSequence=",g_sequence," | failCount=",g_fail_count);}
