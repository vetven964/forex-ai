...
void TrailOurPositions()
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      if(profit<InpTrailTrigger)continue;
      ...
      {
         desired=MathMax(bid-InpTrailPoints*point,open+lockOffset);
         desired=NormalizeDouble(desired,digits);
         if(sl==0||desired>sl+point)trade.PositionModify(ticket,desired,tp);
      }
      else if(type==POSITION_TYPE_SELL)
      {
         desired=MathMin(ask+InpTrailPoints*point,open-lockOffset);
         desired=NormalizeDouble(desired,digits);
         if(sl==0||desired<sl-point)trade.PositionModify(ticket,desired,tp);
      }
   }
