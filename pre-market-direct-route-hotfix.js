/* V-TRADE AI — Pre-Market direct route hotfix V5
 * FIX: use canonical MT5 candles first, with parseBrokerCandles() fallback.
 ...
 */
'use strict';
const MARKER='VTRADE_PREMARKET_DIRECT_ROUTE_HOTFIX_V5_MT5_PARSE_FALLBACK';
...
/* ${MARKER} */
(function installPreMarketDirectRouteV5(app){
  if(!app||app.__VTRADE_PREMARKET_DIRECT_ROUTE_V5__) return;
  app.__VTRADE_PREMARKET_DIRECT_ROUTE_V5__=true;
  ...
  function safeParse(tf){try{if(typeof parseBrokerCandles==='function'){const a=parseBrokerCandles(tf);if(Array.isArray(a)&&a.length)return a;}}catch(e){console.warn('[V-TRADE PRE-MARKET] parseBrokerCandles fallback failed',tf,e?.message||e);}return [];}
  function frame(a,tf,price){
    ...
    const feedBars=arrOf(feed);
    const parsedBars=safeParse(tf);
    const nodeBars=arrOf(node);
    const raw=feedBars.length>=30?feedBars:(parsedBars.length>=30?parsedBars:(feedBars.length?feedBars:(nodeBars.length?nodeBars:parsedBars)));
    ...
    return {...node,tf,ready:candles.length>=30,bars:candles.length,candles,bars:candles,open,high,low,close,currentPrice:p,bias,directionScore:score,score,feedBars:candles.length,sourceBars:feedBars.length,parsedBars:parsedBars.length,candle:{...(node?.candle||{}),open,high,low,close},workflow:{entryAuthorization:false,telegramIndependent:true}};
  }
  function calculate(core){
    const a=core?.analysis||core?.data||core?.result||core||{};
    let live=n(a?.livePrice??a?.price);
    if(live==null){try{live=n(typeof brokerLivePrice==='function'?brokerLivePrice()?.price:null);}catch(_){} }
    if(live==null&&typeof brokerFeed!=='undefined')live=n(brokerFeed?.quote?.last??brokerFeed?.price);
  ...
  console.log('[V-TRADE PRE-MARKET] DIRECT ROUTE V5 ACTIVE | MT5 brokerFeed + parseBrokerCandles fallback | CORE M5/M15/H1/H4 | D1 optional');
})(app);
`;
