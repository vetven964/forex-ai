/* V-TRADE AI — Pre-Market direct route hotfix V7
 * Uses canonical MT5 brokerFeed candles, with parseBrokerCandles() fallback.
 * Replaces older injected V4/V5/V6 blocks to avoid Express first-match conflicts.
 * Core: M5/M15/H1/H4. D1 optional/diagnostic. Analysis-only.
 */
'use strict';
const MARKER='VTRADE_PREMARKET_DIRECT_ROUTE_HOTFIX_V7';
const OLD=['VTRADE_PREMARKET_DIRECT_ROUTE_HOTFIX_V4_MT5_FEED','VTRADE_PREMARKET_DIRECT_ROUTE_HOTFIX_V5_MT5_PARSE_FALLBACK','VTRADE_PREMARKET_DIRECT_ROUTE_HOTFIX_V6_MT5_PARSE_FALLBACK'];
...
/* ${MARKER} */
(function installPreMarketDirectRouteV7(app){
 if(!app||app.__VTRADE_PREMARKET_DIRECT_ROUTE_V7__)return;
 app.__VTRADE_PREMARKET_DIRECT_ROUTE_V7__=true;
 ...
 async function handler(req,res){cors(req,res);res.set('Cache-Control','no-store');if(req.method==='OPTIONS')return res.status(204).end();try{if(typeof buildXauAnalysis!=='function')throw new Error('Canonical XAUUSD analysis function unavailable');const r=calc(await buildXauAnalysis());console.log('[V-TRADE PRE-MARKET] V7 MT5 OK | core='+r.available+'/4 | M5='+r.timeframes.M5.bars+' M15='+r.timeframes.M15.bars+' H1='+r.timeframes.H1.bars+' H4='+r.timeframes.H4.bars+' | D1='+(r.optionalD1?'READY':'OPTIONAL')+' | price='+r.livePrice);return res.json(r);}catch(e){console.error('[V-TRADE PRE-MARKET] V7 ERROR:',e?.stack||e?.message||e);return res.status(502).json({success:false,error:String(e?.message||e),workflow:{entryAuthorization:false,telegramIndependent:true}});}}
 ...
 console.log('[V-TRADE PRE-MARKET] DIRECT ROUTE V7 ACTIVE | MT5 brokerFeed + parseBrokerCandles fallback | CORE M5/M15/H1/H4');
})(app);
`;
