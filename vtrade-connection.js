...
(() => {
  ...
  // Persistent account/logout layer for the legacy premium terminal.
  // It intentionally skips public/auth/admin pages so those pages keep their own UI.
  function installAccountLayer() {
    ...
    css.textContent = `
      ...
      .vtrade-account-role{display:block;color:#8493ab;font-size:9px;font-weight:500;margin-top:2px}
      .vtrade-chevron{color:#8493ab;font-size:10px}
      ...
      .vtrade-menu.show{display:block;animation:vtradeMenuIn .16s ease}
      @keyframes vtradeMenuIn{from{opacity:0;transform:translateY(-5px)}to{opacity:1;transform:none}}
      .vtrade-menu-head{padding:9px 10px 11px;border-bottom:1px solid #1b2a41;margin-bottom:5px}
      .vtrade-menu-head b{font-size:12px}.vtrade-menu-head small{display:block;color:#8493ab;margin-top:3px;font-size:10px}
      ...
      .vtrade-menu a:hover,.vtrade-menu button:hover{background:#111d30;color:#fff}
      .vtrade-menu .logout{color:#ff7c89}.vtrade-menu .logout:hover{background:#2b0c13}
      .vtrade-menu .hidden{display:none}
    ...
    layer.id = 'vtradeAccountLayer';
    layer.className = 'vtrade-account';
    layer.innerHTML = `
      <button class="vtrade-account-btn" type="button" aria-expanded="false" aria-label="Account menu">
        <span class="vtrade-avatar" id="vtradeAvatar">V</span>
        <span class="vtrade-account-name"><span id="vtradeAccountName">Account</span><span class="vtrade-account-role" id="vtradeAccountRole">Session</span></span>
        <span class="vtrade-chevron">⌄</span>
      </button>
      <div class="vtrade-menu" id="vtradeAccountMenu">
        <div class="vtrade-menu-head"><b id="vtradeMenuName">Account</b><small id="vtradeMenuEmail">Authenticated session</small></div>
        <a href="index.html">⌂ Home</a>
        <a href="connection.html">◉ Connection Center</a>
        <a id="vtradeAdminLink" class="hidden" href="admin-dashboard.html">♛ Admin Command Center</a>
        <button class="logout" id="vtradeLogout" type="button">↪ Sign out</button>
      </div>`;

    top.appendChild(layer);

    const btn = layer.querySelector('.vtrade-account-btn');
    const menu = layer.querySelector('#vtradeAccountMenu');
    ...
    const applyUser = (user) => {
      ...
      if (role === 'admin') layer.querySelector('#vtradeAdminLink').classList.remove('hidden');
    };
    ...
    layer.querySelector('#vtradeLogout').addEventListener('click', async () => {
      const logout = layer.querySelector('#vtradeLogout');
  ...
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', installAccountLayer);
  else installAccountLayer();
})();
