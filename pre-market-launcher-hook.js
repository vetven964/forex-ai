...
(function installPreMarketCandleOpen(app){
 ...
 const getBars=(a,tf)=>{const t=a?.timeframes||a?.mtf||a?.multiTimeframe||{};const x=t[tf]??t[tf.toLowerCase()]??a?.[tf]??a?.[tf.toLowerCase()];if(Array.isArray(x))return x;if(Array.isArray(x?.bars))return x.bars;if(Array.isArray(x?.candles))return x.candles;return []};
 ...
 function analyze(a,tf,price){
  ...
  const openMove=(p-x.o)/range, body=(x.c-x.o)/range, mom=(prev.c-prev.o)/range;
  const upper=Math.max(0,x.h-Math.max(x.o,x.c)), lower=Math.max(0,Math.min(x.o,x.c)-x.l);
  const rejection=(lower-upper)/range;
  const prior=r.slice(-9,-1), ph=Math.max(...prior.map(b=>b.h)), pl=Math.min(...prior.map(b=>b.l));
  const structure=x.c>ph?1:x.c<pl?-1:x.c>prev.h?.5:x.c<prev.l?-.5:0;
  ...
  return {tf,ready:true,buyPct:buy,sellPct:Math.round((100-buy)*10)/10,bias:buy>50?'BULLISH':buy<50?'BEARISH':'NEUTRAL',open:x.o,currentPrice:p,openMove:p-x.o,avgRange:avg,evidence:{body:Math.round(body*1000)/10,momentum:Math.round(mom*1000)/10,rejection:Math.round(rejection*1000)/10,structure}};
 }
 function calculate(raw){
  const a=raw?.analysis||raw?.data||raw||{}, price=n(a?.price??a?.livePrice??a?.quote?.price??a?.mt5?.price);const rows={};let b=0,s=0,w=0,ready=0;
  for(const tf of frames){const x=analyze(a,tf,price);rows[tf]=x;if(x.ready){ready++;b+=x.buyPct*weights[tf];s+=x.sellPct*weights[tf];w+=100*weights[tf];}}
  const buy=w?Math.round(clamp(b/w*100)*10)/10:null, sell=buy==null?null:Math.round((100-buy)*10)/10;
 ...
 async function handler(req,res){cors(req,res);if(req.method==='OPTIONS')return res.status(204).end();try{const token=String(req.get('x-vtrade-auth')||'');const port=Number(process.env.PORT||10000);const r=await fetch('http://127.0.0.1:'+port+'/api/analysis/xauusd',{headers:token?{'x-vtrade-auth':token}:{},signal:AbortSignal.timeout(12000)});const raw=await r.json().catch(()=>({success:false,error:'Invalid analysis response'}));if(!r.ok||raw?.success===false)return res.status(r.status||502).json({success:false,error:raw?.error||'MT5 analysis unavailable'});return res.json(calculate(raw));}catch(e){return res.status(502).json({success:false,error:String(e?.message||e)});}}
...
Module._extensions['.js']=function(fileMod,filename){
 ...
 let patched=inject(originalRead(filename,'utf8'));
 const old=fs.readFileSync;
 fs.readFileSync=function(file,...args){if(path.resolve(String(file))===SERVER_FILE)return patched;return originalRead(file,...args)};
