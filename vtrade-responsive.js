/* V TRADE AI — Mobile App shell / navigation V8 */
(() => {
  'use strict';
  if (window.__VTRADE_RESPONSIVE_V8__) return;
  window.__VTRADE_RESPONSIVE_V8__ = true;
  const isMobile=()=>window.matchMedia('(max-width:900px)').matches;
  const path=location.pathname.split('/').pop().toLowerCase();
  function injectStyle(){
    if(document.getElementById('vtradeResponsiveRuntimeStyleV8'))return;
    const s=document.createElement('style');s.id='vtradeResponsiveRuntimeStyleV8';
  ...
  function loadDirectPhoneShell(){
    ...
    const s=document.createElement('script');s.id='vtradeDirectPhoneShellScript';s.src='vtrade-phone-shell-direct-v1.js?v=20260822-direct2';document.head.appendChild(s);
  }
