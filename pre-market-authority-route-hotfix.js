...
(function installPreMarketAuthorityV1(app){
 ...
 const TFS=['M5','M15','H1','H4','D1'],CORE=['M5','M15','H1','H4'],WEIGHTS={M5:1,M15:2,H1:3,H4:4,D1:5};
 ...
 function score(c){if(!Array.isArray(c)||c.length<3)return{buy:50,sell:50,bias:'NEUTRAL',score:0};const last=c[c.length-1],prev=c[c.length-2],look=c.slice(-20),hi=Math.max(...look.map(x=>x.h)),lo=Math.min(...look.map(x=>x.l)),range=Math.max(last.h-last.l,1e-9),body=last.c-last.o;let bull=50,bear=50;if(body>0)bull+=Math.min(15,15*Math.abs(body)/range);if(body<0)bear+=Math.min(15,15*Math.abs(body)/range);if(last.c>prev.c)bull+=7;else if(last.c<prev.c)bear+=7;if(last.c>(hi+lo)/2)bull+=10;else bear+=10;const closes=c.slice(-Math.min(50,c.length)).map(x=>x.c);if(closes.length>=20){const fast=closes.slice(-10).reduce((a,b)=>a+b,0)/10;const slow=closes.reduce((a,b)=>a+b,0)/closes.length;if(fast>slow)bull+=13;else if(fast<slow)bear+=13;}const total=Math.max(1,bull+bear),buy=clamp(bull/total*100),sell=clamp(100-buy);return{buy,sell,bias:buy>sell?'BULLISH':sell>buy?'BEARISH':'NEUTRAL',score:Math.round(Math.max(buy,sell))};}
 function handler(req,res){res.set('Cache-Control','no-store');const q=live();const timeframes={};let weighted=0,totalWeight=0;for(const tf of TFS){const b=bars(tf),last=b.bars[b.bars.length-1]||{},s=score(b.bars);timeframes[tf]={tf,ready:b.bars.length>=30,bars:b.bars.length,candles:b.bars,source:b.source,open:last.o??null,high:last.h??null,low:last.l??null,close:last.c??null,currentPrice:q??last.c??null,buyPct:Math.round(s.buy),sellPct:Math.round(s.sell),direction:s.bias,score:s.score};if(timeframes[tf].ready){weighted+=s.buy*WEIGHTS[tf];totalWeight+=100*WEIGHTS[tf];}}const ready=CORE.filter(tf=>timeframes[tf].ready).length;const buy=totalWeight?weighted/totalWeight*100:50,sell=100-buy,bias=buy>sell?'BULLISH':sell>buy?'BEARISH':'NEUTRAL',confidence=Math.round(50+Math.abs(buy-sell)/2);return res.json({success:true,symbol:'XAUUSD',source:'MT5 brokerFeed',price:q??timeframes.M5.currentPrice??null,livePrice:q??timeframes.M5.currentPrice??null,available:ready,required:4,complete:ready===4,optionalD1:timeframes.D1.ready,missingTimeframes:CORE.filter(tf=>!timeframes[tf].ready),timeframes,frames:timeframes,buyStrengthPct:Math.round(buy),sellStrengthPct:Math.round(sell),bias,directionScore:Math.round(Math.max(buy,sell)),preAiConfidence:confidence,confidence,gates:{liquiditySweep:false,mss:false,bos:false,displacement:false,fvg:false,orderBlock:false,premiumDiscountOk:false,executionZone:false,technicalMomentumOk:false,spreadOk:false,allGatesPassed:false},ict:{},confirmations:{},zone:{authorization:false},workflow:{stage:ready===4?'PRE_MARKET_MTF_READY':'PRE_MARKET_MTF_WAITING',source:'MT5_AUTHORITATIVE',entryAuthorization:false,orderAuthorization:false,aiRole:'CONFIRMATION_ONLY',telegramIndependent:true},generatedAt:new Date().toISOString()});}
